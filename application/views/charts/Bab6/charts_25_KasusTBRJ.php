...
for ($j = 0; $j < $rs_count; $j++) {
    $ind = 0;
    for ($k = 0; $k < 7; $k++) {
        $totalN2[$ind] += $row[$track]->KASUS_TB_RJ_N2;
        $totalN1[$ind] += $row[$track]->KASUS_TB_RJ_N1;
        $totalN[$ind] += $row[$track]->KASUS_TB_RJ_N;
        $ind++;
        $track++;
    }
}
...
<script>
    (function basic(container) {
        ...
        graph = Flotr.draw(container, [
            ...
            shadowSize: 0,
            title: "Kasus TB Rawat Jalan Tahun <?php echo $yearName; ?>",
            parseFloat: false,
            xaxis: {
                title: "Tahun",
                min: 0,
