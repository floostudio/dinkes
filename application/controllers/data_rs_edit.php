...
class data_rs_edit extends CI_Controller {
    ...
    public function index() {
        if ($this->user_id) {
			...
			if($idRS == null)
			    redirect("data_rs");
			$data['idcek'] = $idRS;
            $data['rumah_sakit'] = $this->m_rumahsakit->retrieveRSbyId($idRS);
            ...
            $this->load->view('data_rs_edit', $data);
        } else {
            redirect("login");
        }
    }
   ...
   public function update() {
	...
	$active= $this->input->post('rs_stat');
        $pemilik= $this->input->post('rs_pemilik');
		
        $status = $this->m_rumahsakit->editRSAll($rs_id, $rsnoreg, $region, $nama, $alamat, //new
                ...
                $strength, $weakness,$active,$pemilik);
